...
- Easy (but not entirely accurate): $P_{TOT} >= P_{LH}$ 
    ...
    - This is prone to the following artifacts: 
        - When a PINI drops, the $P_{TOT} >= P_{LH}$ can drop below 1, even though we are still in H-mode. 
        - Near rampdown, any number of the parameters could spike, (see JET #86540) 
        - At the very beginning, there is a spike in density __?from the gas injection?__
    - Additional flag of $45 < t < 60$ for JET. 
...
2. times of the window $t$ exist between $44 < t < 60$ (for JET)
    - This is temporary, but should exclude times during rampdown and rampup where the LH threshold can occur (although this should be investigated?)
