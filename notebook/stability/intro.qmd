...
- $\nu_{*j}=\frac{\nu_j}{\epsilon \omega_{bj}}$
  - where $\nu_e = \tau_e^{-1}$, $\nu_i = \tau_i^{-1}$ and $\omega_{bj} = \epsilon^{1/2}(T_j/m_j)^{1/2} / Rq$
    - where the electron collision time for electron collisions with ions is defined as $\tau_e = \frac{12\pi^{3/2}}{\sqrt{2}}\frac{\epsilon_0 m_e^{1/2}T_e^{3/2}}{n_iZ^2e^4\ln \Lambda}$ but can be simplified for singly charged ions to be $6.4\times 10^{14} \frac{T_e^{3/2}}{n}$, with $\ln \Lambda$ and $T_e$ in keV.
...
pe = ne*te*(1.602e-19) 
times = [3900 + i for i in range(500)]
ts = [time[t] for t in times]
```

```{python}
mu_0 = 1.256e-6 
def boostrap_current(slice_pressure: np.ndarray, slice_temperature: np.ndarray, slice_density: np.ndarray, poloidal_field: float, major_radius: float, minor_radius: float): 
    aspect_ratio = minor_radius / major_radius
    x = np.sqrt(2*aspect_ratio)
    D = 2.4 + 5.4*x + 2.6*x
    collision_electron = 0.0 
    collision_ion = 0.0
    pass 
```
```{python}
...
def find_alpha_max_from_pressure_slice(slice_pressure: np.ndarray, slice_radius: np.ndarray):
    pressure_grad = -np.diff(slice_pressure/ 1000.0)
    pg_sorted_max_bool_vals_in_ped = np.logical_and(slice_radius[1:] > 0.8, slice_radius[1:] < 1.1)[np.argsort(pressure_grad)]
    in_ped = [x for i, x in enumerate(np.argsort(pressure_grad)) if pg_sorted_max_bool_vals_in_ped[i]]
    max_pressure_grad_idx = in_ped[-1]
    return pressure_grad[max_pressure_grad_idx], slice_radius[1:][max_pressure_grad_idx], slice_pressure[1:][max_pressure_grad_idx]

def calculate_boostrap_current_slice(slice_pressure: np.ndarray, slice_temperature: np.ndarray, slice_radius: np.ndarray, total_current: float, toroidal_field: float, calc_type: str = 'simple'): 
