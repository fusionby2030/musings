...
![MLE Bo implementation](./figures/MLE_BO_FULL.png)
...
![MCMC over posterior](./figures/MCMC_INVERSE_UQ.png)

Additionally, we can use this to gather _inverse_ uncertainty about how the model is affected due to changes in $\theta$. 
...
plt.scatter(x_range, experimental_result)

for alpha, beta in zip(alpha_sample, beta_sample): 
  plt.plot(x_range, model_f(x_range, alpha.item(), beta.item()), label=r'$\alpha$={alpha:.4}; $\beta$={beta:.4}'.format(alpha=alpha, beta=beta))
plt.show()
